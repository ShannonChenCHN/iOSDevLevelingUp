...
```
        ...
        // 点击搜索按钮
        [navigationBar setDidSelectSearchButton:^(IXSearchNavigationBar *searchBar) {
        [searchBar.searchField resignFirstResponder]; // 这句代码导致了内存泄漏
        [weakSelf showHUD];
        [weakSelf.apiManager refreshData];
    }];
```
