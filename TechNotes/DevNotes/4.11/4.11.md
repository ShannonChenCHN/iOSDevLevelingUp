...
- 在哪里埋点？
  ...
  - B. 处理点击事件的地方
- 数据怎么传？
  埋点是与用户触发事件相关的，其实也就是与 view 相关，你可能会想数据决定一切，所以我们可以通过 model 传进去， 这样做没有问题，但是 model 是可复用的，一个 view 对应的 model 可能不止一种 model，因此最好的方式不是在 model 中传进去，而是通过给 view 添加相关的属性会比较好，或者借助 protocol 的方式通过统一接口的 model 传给 view，也是可以的
