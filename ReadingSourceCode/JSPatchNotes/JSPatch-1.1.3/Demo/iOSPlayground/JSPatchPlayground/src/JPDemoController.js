include('Child.js');
require('UILabel, UIColor, UIFont, UIScreen, UIImageView, UIImage', 'UIButton', 'NSMutableArray', 'JPRootViewController') // 导入 Objective-C 系统类
...
// 定义实例方法
{
    viewDidLoad: function() {
        ...
        var button = UIButton.alloc().initWithFrame({x: 0, y: 400, width: screenWidth, height: 50});
        button.setTitle_forState(self.jsTitle(), 0);
        button.setBackgroundColor(UIColor.redColor());
        button.addTarget_action_forControlEvents(self, 'didSelectButton', 1 << 6);
...
//        JPDemoController.showAlert();
    ...
    didSelectButton : function () {
            _OC_log('didSelectButton');
            self.testSelfSuper();
    },

    testSelfSuper: function() {
          var child = Child.new();
          child.two();
