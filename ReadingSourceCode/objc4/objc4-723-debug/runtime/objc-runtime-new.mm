...
getMethodNoSuper_nolock(Class cls, SEL sel)
{
    ...
    // 类中数据的方法列表 methods 是一个二维数组 method_array_t
                       ...
                       bool initialize, bool cache, bool resolver)
{
    ...
    // 如果有缓存，就从缓存中查找
    // 实际上因为 _class_lookupMethodAndLoadCache3 传入的 cache = NO，所以这里会直接跳过 if 中代码的执行，在 objc_msgSend 中已经使用汇编代码查找过了
    if (cache) {
    ...
    // 在 isRealized 和 isInitialized 期间进行加锁，保证方法查找（method-lookup）以及缓存填充（cache-fill）的原子性
    // 保证在运行以下代码时不会因为 category 添加了新方法导致缓存被冲洗（flush）
    runtimeLock.read();
 ...
 retry:    
    ...
    // 在当前类中查找方法实现的缓存：从 objc_class 的 cache 变量中获取选择子对应的实现
    imp = cache_getImp(cls, sel);
    if (imp) goto done;
             ...
             curClass = curClass->superclass)
        {
            ...
            if (imp) {
                ...
                if (imp != (IMP)_objc_msgForward_impcache) {
                    ...
                    // 如果找到了，而且不是在父类中转发的，就将它加入当前类的缓存中
                    ...
                    // 如果是在父类中转发的，就停止查找，后面再交给 resolver 进行方法决议
                    break;
                }
            }
            
            // Superclass method list.
            // 如果没有命中缓存，就从父类的方法列表中寻找方法的实现
            Method meth = getMethodNoSuper_nolock(curClass, sel);
            if (meth) {
                // 如果在这里找到了方法的实现，将它加入当前类的缓存中
    ...
    if (resolver  &&  !triedResolver) {
        runtimeLock.unlockRead();
        // 方法决议，也就是判断当前类是否实现了 resolveInstanceMethod: 或者 resolveClassMethod: 方法，然后用 objc_msgSend 执行上述方法
        _class_resolveMethod(cls, sel, inst);
        runtimeLock.read();
        // Don't cache the result; we don't hold the lock so it may have 
        // changed already. Re-do the search from scratch instead.
        // 这里不要缓存结果，因为有可能会发生变化
        // 跳转到 retry 标签，重新执行查找方法实现的流程，只不过不会再调用 resolveInstanceMethod: 方法了
        triedResolver = YES;
        goto retry;
    }
